...
适用于芝加哥数据集：144 轮（10 分钟切片）
多次重复实验取平均，输出平均结果到原文件名。
输入：experiment3_worker_segments.json, experiment3_task_segments.json
输出：experiment3_final_result_ours.json, experiment3_step2_ours_taskcover.json,
      experiment3_step2_ours_cumulative_trusted_ratio.json,
      experiment3_step2_ours_std_results.json（标准差）
额外输出：experiment3_step2_ours_taskcover.csv, experiment3_step2_ours_cumulative_trusted_ratio.csv,
         experiment3_step2_worker_category.csv
"""
...
def data_preparation(worker_segments_path, task_segments_path,
    ...
    save_json(lgsc_params, 'experiment3_lgsc_params_ours.json')
    print("已保存 LGSC 参数 experiment3_lgsc_params_ours.json")
...
def generate_validation_tasks(workers, task_grid_map, task_time_map, Uc, Uu, round_idx, M):
    ...
    for w in sorted(available_workers, key=lambda x: x['worker_id']):
        wid = w['worker_id']
        for task in w['covered_tasks']:
            if task['task_start_time'] // SLOT_SEC != round_idx:
                continue
            tid = task['task_id']
...
def greedy_recruitment_ours(workers, task_covered_count, required_workers, total_learned_counts,
    ...
    for r in range(R):
        ...
        for wid in new_member_set:
            ...
            member_stats[wid]['total_fee'] += PGRD_PARAMS['fee']
        ...
        bonus_paid, avg_sunk_loss, avg_roi_lgsc, members_above = lgsc_payment(
        ...
        round_details.append({
            ...
            'avg_roi_this_round': round(avg_roi_lgsc, 2),
...
def run_experiment_ours(seed, worker_segments_path, task_segments_path,
    ...
    OUTPUT_WORKER_OPTIONS = 'experiment3_worker_option_set_ours.json'
    OUTPUT_TASK_WEIGHTS = 'experiment3_task_weight_list_ours.json'
    OUTPUT_TASK_GRID = 'experiment3_tasks_grid_num_ours.json'
    OUTPUT_TASK_CLASS = 'experiment3_tasks_classification_ours.json'
    ...
    Uc, Uu, Um, R_m, R_n, task_time_map = initialize_cmab(
        OUTPUT_WORKER_OPTIONS, OUTPUT_TASK_WEIGHTS, OUTPUT_TASK_CLASS, 'experiment3_lgsc_params_ours.json'
    )
...
# ========== 主函数：多次重复实验取平均 ==========
def main():
    WORKER_SEGMENTS = 'experiment3_worker_segments.json'
    TASK_SEGMENTS = 'experiment3_task_segments.json'
    ...
    all_roi = []
    all_unit_costs = []   # 存储每次实验的单位任务成本
    all_total_rewards = []   # 新增

    # 定义要删除的中间文件列表
    intermediate_files = [
        'experiment3_worker_option_set_ours.json',
        'experiment3_task_weight_list_ours.json',
        'experiment3_tasks_grid_num_ours.json',
        'experiment3_tasks_classification_ours.json',
        'experiment3_lgsc_params_ours.json'
    ]

    print("获取总任务数...")
    temp_seed = seeds[0]
    random.seed(temp_seed)
    temp_worker_options = 'experiment3_worker_option_set_ours.json'
    temp_task_weights = 'experiment3_task_weight_list_ours.json'
    temp_task_grid = 'experiment3_tasks_grid_num_ours.json'
    temp_task_class = 'experiment3_tasks_classification_ours.json'
    ...
    for idx, seed in enumerate(seeds):
        ...
        all_total_rewards.append(result['total_worker_reward'])   # 新增
        ...
        all_roi.append(result['avg_member_roi'])
    ...
    avg_total_reward = np.mean(all_total_rewards)          # 新增
    std_total_reward = np.std(all_total_rewards)           # 新增

    # 工人类别 CSV
    with open("experiment3_step2_worker_category.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 工人类别变化曲线已保存至 experiment3_step2_worker_category.csv")
    ...
    save_json(avg_coverage_records, "experiment3_step2_ours_taskcover.json")
    with open("experiment3_step2_ours_taskcover.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_cumulative_records, "experiment3_step2_ours_cumulative_trusted_ratio.json")
    with open("experiment3_step2_ours_cumulative_trusted_ratio.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    avg_result = {
        ...
        'total_worker_reward': round(avg_total_reward, 2),   # 新增
    ...
    save_json(avg_result, "experiment3_final_result_ours.json")
    print("✅ 平均最终结果已保存至 experiment3_final_result_ours.json")
    ...
    std_result = {
        ...
        'std_total_worker_reward': round(std_total_reward, 2),   # 新增
        ...
        "std_avg_roi": round(std_roi, 4),
        "std_unit_cost": round(std_unit_cost, 2)
    }
    save_json(std_result, "experiment3_step2_ours_std_results.json")
    print("✅ 标准差结果已保存至 experiment3_step2_ours_std_results.json")

    # 将标准差结果保存为 CSV
    with open("experiment3_step2_ours_std_results.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["metric", "value"])
        for key, value in std_result.items():
            if isinstance(value, list):
                for idx, v in enumerate(value):
                    writer.writerow([f"{key}_round_{idx}", v])
            else:
                writer.writerow([key, value])
    print("✅ 标准差结果已同时保存至 experiment3_step2_ours_std_results.csv")
