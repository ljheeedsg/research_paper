...
输出：experiment3_final_result_B4.json, experiment3_step1_B4_taskcover.json,
      experiment3_step1_B4_cumulative_trusted_ratio.json,
      experiment3_step1_B4_std_results.json（标准差）
      experiment3_step1_B4_worker_category.csv（工人类别变化）
"""
...
def data_preparation(worker_segments_path, task_segments_path,
    ...
    save_json(lgsc_params, 'experiment3_lgsc_params_B4.json')
    print("已保存 LGSC 参数 experiment3_lgsc_params_B4.json")
...
def greedy_recruitment_B4(workers, task_covered_count, required_workers, total_learned_counts,
    ...
    print(f"活跃会员数: {len(roi_list)}, 非活跃会员数: {inactive_members}, 平均ROI: {avg_roi:.4f}")
    result = {
...
# ========== 单次实验封装 ==========
def run_experiment_B4(seed, worker_segments_path, task_segments_path,
                      budget, K, R, M_VERIFY, ETA, THETA_HIGH, THETA_LOW,
                      pgrd_params, member_validity):
    random.seed(seed)

    # 使用 experiment3 前缀的临时文件
    OUTPUT_WORKER_OPTIONS = 'experiment3_worker_option_set_B4.json'
    OUTPUT_TASK_WEIGHTS = 'experiment3_task_weight_list_B4.json'
    OUTPUT_TASK_GRID = 'experiment3_tasks_grid_num_B4.json'
    OUTPUT_TASK_CLASS = 'experiment3_tasks_classification_B4.json'
    ...
    Uc, Uu, Um, R_m, R_n, task_time_map = initialize_cmab(
        OUTPUT_WORKER_OPTIONS, OUTPUT_TASK_WEIGHTS, OUTPUT_TASK_CLASS, 'experiment3_lgsc_params_B4.json'
    )

    task_class = load_json(OUTPUT_TASK_CLASS)
    task_grid_map = {item['task_id']: item['grid_id'] for item in load_json(OUTPUT_TASK_GRID)}

    result, coverage_curve, cumulative_curve, worker_cat = greedy_recruitment_B4(
        workers, task_covered_count, required_workers, total_learned_counts,
        Uc, Uu, Um, R_m, R_n,
...
def main():
    ...
    # 定义要删除的中间文件列表
    intermediate_files = [
        'experiment3_worker_option_set_B4.json',
        'experiment3_task_weight_list_B4.json',
        'experiment3_tasks_grid_num_B4.json',
        'experiment3_tasks_classification_B4.json',
        'experiment3_lgsc_params_B4.json'
    ]
    ...
    temp_worker_options = 'experiment3_worker_option_set_B4.json'
    temp_task_weights = 'experiment3_task_weight_list_B4.json'
    temp_task_grid = 'experiment3_tasks_grid_num_B4.json'
    temp_task_class = 'experiment3_tasks_classification_B4.json'
    ...
    for idx, seed in enumerate(seeds):
        ...
        coverage_curve, cumulative_curve, result, worker_cat = run_experiment_B4(
    ...
    save_json(avg_coverage_records, "experiment3_step1_B4_taskcover.json")
    with open("experiment3_step1_B4_taskcover.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_cumulative_records, "experiment3_step1_B4_cumulative_trusted_ratio.json")
    with open("experiment3_step1_B4_cumulative_trusted_ratio.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_result, "experiment3_final_result_B4.json")
    print("✅ 平均最终结果已保存至 experiment3_final_result_B4.json")
    ...
    save_json(std_result, "experiment3_step1_B4_std_results.json")

    # 工人类别变化 CSV
    with open("experiment3_step1_B4_worker_category.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 工人类别变化曲线已保存至 experiment3_step1_B4_worker_category.csv")
