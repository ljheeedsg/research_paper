...
输出：experiment2_final_result_ours.json, experiment2_step2_ours_taskcover.json,
      experiment2_step2_ours_cumulative_trusted_ratio.json,
      experiment2_step2_ours_std_results.json（标准差）
额外输出：experiment2_step2_ours_taskcover.csv, experiment2_step2_ours_cumulative_trusted_ratio.csv,
         experiment2_step2_worker_category.csv
"""
...
# LGSC 参数
SUNK_THRESHOLD = 30         # 改为 30（原为 20）
MEMBER_BONUS = 20           # 改为 20（原为 50）
RHO_INIT = 1.0
...
def data_preparation(worker_segments_path, task_segments_path,
    ...
    save_json(lgsc_params, 'experiment2_lgsc_params_ours.json')
    print("已保存 LGSC 参数 experiment2_lgsc_params_ours.json")
...
def run_experiment_ours(seed, worker_segments_path, task_segments_path,
    ...
    OUTPUT_WORKER_OPTIONS = 'experiment2_worker_option_set_ours.json'
    OUTPUT_TASK_WEIGHTS = 'experiment2_task_weight_list_ours.json'
    OUTPUT_TASK_GRID = 'experiment2_tasks_grid_num_ours.json'
    OUTPUT_TASK_CLASS = 'experiment2_tasks_classification_ours.json'
    ...
    Uc, Uu, Um, R_m, R_n, task_time_map = initialize_cmab(
        OUTPUT_WORKER_OPTIONS, OUTPUT_TASK_WEIGHTS, OUTPUT_TASK_CLASS, 'experiment2_lgsc_params_ours.json'
    )
...
def main():
    ...
    # 定义要删除的中间文件列表
    intermediate_files = [
        'experiment2_worker_option_set_ours.json',
        'experiment2_task_weight_list_ours.json',
        'experiment2_tasks_grid_num_ours.json',
        'experiment2_tasks_classification_ours.json',
        'experiment2_lgsc_params_ours.json'
    ]

    print("获取总任务数...")
    temp_seed = seeds[0]
    random.seed(temp_seed)
    temp_worker_options = 'experiment2_worker_option_set_ours.json'
    temp_task_weights = 'experiment2_task_weight_list_ours.json'
    temp_task_grid = 'experiment2_tasks_grid_num_ours.json'
    temp_task_class = 'experiment2_tasks_classification_ours.json'
    ...
    # 工人类别 CSV
    with open("experiment2_step2_worker_category.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 工人类别变化曲线已保存至 experiment2_step2_worker_category.csv")
    ...
    save_json(avg_coverage_records, "experiment2_step2_ours_taskcover.json")
    with open("experiment2_step2_ours_taskcover.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_cumulative_records, "experiment2_step2_ours_cumulative_trusted_ratio.json")
    with open("experiment2_step2_ours_cumulative_trusted_ratio.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_result, "experiment2_final_result_ours.json")
    print("✅ 平均最终结果已保存至 experiment2_final_result_ours.json")
    ...
    save_json(std_result, "experiment2_step2_ours_std_results.json")
    print("✅ 标准差结果已保存至 experiment2_step2_ours_std_results.json")

    # 将标准差结果保存为 CSV
    with open("experiment2_step2_ours_std_results.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 标准差结果已同时保存至 experiment2_step2_ours_std_results.csv")
