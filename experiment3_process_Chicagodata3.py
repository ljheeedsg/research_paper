...
# ==================== 配置 ====================
VEHICLE_FILE = 'experiment3_vehicle.csv'          # 车辆轨迹文件（芝加哥数据）
TASK_CSV = 'experiment3_tasks.csv'                # 输出任务 CSV
TASK_JSON = 'experiment3_task_segments.json'      # 输出任务 JSON
...
def main():
    ...
    # 先读取一次以获取列名并确定车辆ID列
    with open(VEHICLE_FILE, 'r', encoding='utf-8-sig') as f:  # 使用 utf-8-sig 处理 BOM
        reader = csv.DictReader(f)
        # 清理列名（去除空格）
        fieldnames = [name.strip() for name in reader.fieldnames]
        print("CSV 列名（清理后）:", fieldnames)
        
        # 寻找车辆ID列
        vehicle_key = None
        for possible_key in ['vehicle_id', 'ride_id', 'Vehicle_id', 'Ride_id']:
            if possible_key in fieldnames:
                vehicle_key = possible_key
                break
        if vehicle_key is None:
            raise KeyError(f"未找到车辆ID列，可用的列名：{fieldnames}")
        print(f"使用车辆ID列名: {vehicle_key}")
    
    # 重新打开文件正式读取
    with open(VEHICLE_FILE, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        # 清理字典的键
        for row in reader:
