...
输出：experiment1_step2_final_result_ours.json, experiment1_step2_ours_taskcover.json,
      experiment1_step2_ours_cumulative_trusted_ratio.json,
      experiment1_step2_ours_std_results.json（标准差）
额外输出：experiment1_step2_ours_std_results.csv（误差CSV，方便复制）
"""
...
def main():
    ...
    # 工人类别 CSV
    with open("experiment1_step2_worker_category.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 工人类别变化曲线已保存至 experiment1_step2_worker_category.csv")
    ...
    save_json(avg_coverage_records, "experiment1_step2_ours_taskcover.json")
    print("✅ 平均覆盖率曲线已保存至 experiment1_step2_ours_taskcover.json")

    # 保存平均覆盖率曲线（CSV）
    with open("experiment1_step2_ours_taskcover.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 平均覆盖率曲线已保存至 experiment1_step2_ours_taskcover.csv")
    ...
    save_json(avg_cumulative_records, "experiment1_step2_ours_cumulative_trusted_ratio.json")
    print("✅ 平均累积可信任务占比曲线已保存至 experiment1_step2_ours_cumulative_trusted_ratio.json")

    # 保存平均累积可信任务占比曲线（CSV）
    with open("experiment1_step2_ours_cumulative_trusted_ratio.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 平均累积可信任务占比曲线已保存至 experiment1_step2_ours_cumulative_trusted_ratio.csv")
    ...
    save_json(avg_result, "experiment1_step2_final_result_ours.json")
    print("✅ 平均最终结果已保存至 experiment1_step2_final_result_ours.json")
    ...
    save_json(std_result, "experiment1_step2_ours_std_results.json")
    print("✅ 标准差结果已保存至 experiment1_step2_ours_std_results.json")

    # ========== 新增：将标准差结果保存为 CSV（方便复制） ==========
    with open("experiment1_step2_ours_std_results.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 标准差结果已同时保存至 experiment1_step2_ours_std_results.csv")
