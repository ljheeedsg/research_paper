...
输出：experiment3_final_result_B2.json, experiment3_step1_B2_taskcover.json,
      experiment3_step1_B2_cumulative_trusted_ratio.json,
      experiment3_step1_B2_std_results.json（标准差）
      experiment3_step1_B2_worker_category.csv（工人类别变化）
"""
...
def data_preparation(worker_segments_path, task_segments_path,
    ...
    save_json(lgsc_params, 'experiment3_lgsc_params_B2.json')
    print("已保存 LGSC 参数 experiment3_lgsc_params_B2.json（占位）")
...
def greedy_recruitment_B2(workers, task_covered_count, required_workers, total_learned_counts,
                          Uc, Uu, Um, B, K, R, task_time_map, task_class):
    total_cost = 0.0
    remaining_budget = B
    greedy_selected = []
    greedy_rounds = 0
    ...
    return result, task_coverage_records, cumulative_trusted_ratio, worker_category_per_round
...
def run_experiment(seed, worker_segments_path, task_segments_path):
    ...
    OUTPUT_WORKER_OPTIONS = 'experiment3_worker_option_set_B2.json'
    OUTPUT_TASK_WEIGHTS = 'experiment3_task_weight_list_B2.json'
    OUTPUT_TASK_GRID = 'experiment3_tasks_grid_num_B2.json'
    OUTPUT_TASK_CLASS = 'experiment3_tasks_classification_B2.json'
    ...
    Uc, Uu, Um, _, _, task_time_map = initialize_cmab(
        OUTPUT_WORKER_OPTIONS, OUTPUT_TASK_WEIGHTS, OUTPUT_TASK_CLASS, 'experiment3_lgsc_params_B2.json'
    )

    task_class = load_json(OUTPUT_TASK_CLASS)

    result, coverage_records, cumulative_records, worker_cat = greedy_recruitment_B2(
        workers, task_covered_count, required_workers, total_learned_counts,
        Uc, Uu, Um, BUDGET, K, R, task_time_map, task_class
    )
...
def main():
    ...
    # 定义要删除的中间文件列表
    intermediate_files = [
        'experiment3_worker_option_set_B2.json',
        'experiment3_task_weight_list_B2.json',
        'experiment3_tasks_grid_num_B2.json',
        'experiment3_tasks_classification_B2.json',
        'experiment3_lgsc_params_B2.json'
    ]

    # 获取总任务数
    temp_class = 'experiment3_tasks_classification_B2.json'
    temp_worker_opts = 'experiment3_worker_option_set_B2.json'
    temp_task_weights = 'experiment3_task_weight_list_B2.json'
    temp_task_grid = 'experiment3_tasks_grid_num_B2.json'
    data_preparation(WORKER_SEGMENTS, TASK_SEGMENTS,
                     temp_worker_opts, temp_task_weights,
                     temp_task_grid, temp_class)
    task_class_temp = load_json(temp_class)
    TOTAL_TASKS = len(task_class_temp)
    print(f"总任务数: {TOTAL_TASKS}")

    for idx, seed in enumerate(seeds):
        print(f"\n========== 运行实验 {idx+1}/{NUM_SEEDS}，随机种子 {seed} ==========")
        coverage_curve, cumulative_curve, result, worker_cat = run_experiment(seed, WORKER_SEGMENTS, TASK_SEGMENTS)
    ...
    save_json(avg_coverage_records, "experiment3_step1_B2_taskcover.json")
    with open("experiment3_step1_B2_taskcover.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    save_json(avg_cumulative_records, "experiment3_step1_B2_cumulative_trusted_ratio.json")
    with open("experiment3_step1_B2_cumulative_trusted_ratio.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    avg_result = {
        ...
        'init_select': len(load_json(temp_worker_opts)['worker_options']),
        'later_select': int(round(avg_rounds * K)),
        'round_details': [],
        'avg_unit_cost': round(avg_unit_cost, 2)
    }
    save_json(avg_result, "experiment3_final_result_B2.json")
    ...
    save_json(std_result, "experiment3_step1_B2_std_results.json")

    # 工人类别变化 CSV
    with open("experiment3_step1_B2_worker_category.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
    ...
    print("✅ 工人类别变化曲线已保存至 experiment3_step1_B2_worker_category.csv")
