...
OUTPUT_PNG = "experiment2_Fig. 1(d)：每轮RMSE.png"
OUTPUT_PDF = "experiment2_Fig. 1(d)：每轮RMSE.pdf"
OUTPUT_CSV = "experiment2_Fig. 1(d)：每轮RMSE.csv"
SMOOTH_WINDOW = 5
LEGEND_MODE = "manual"
...
def plot_figure():
    ...
    for config in SERIES_CONFIG_RMSE:
        ...
        rounds, per_round_rmse_raw, _ = compute_rmse_series(
            round_results,
            tasks_by_slot,
            task_detail_map,
            trusted_reference_map,
        )
        per_round_rmse = moving_average(per_round_rmse_raw, SMOOTH_WINDOW)

        if csv_rounds is None:
            csv_rounds = rounds
        csv_series_rows.append(per_round_rmse)
        if per_round_rmse:
            max_rmse = max(max_rmse, max(per_round_rmse))
